...
const dialog_form_data = ref<EventUpdate>({
  rid: 0,
  state: 0,
  rid_users: 0,
...
const openDialog = () => {
  dialog_form_data.value = {
    ...
    datetime_end: props.item.event_datetime_end
  }
  dialog.value = true
}

const handleSubmit = (data: EventUpdate) => {
  store_item.updateEvent(data)
  dialog.value = false
}

const handleDelete = () => {
  store_item.deleteEvent(props.item.rid)
  dialog.value = false
}
</script>
